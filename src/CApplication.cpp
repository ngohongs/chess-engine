...
: m_Interface(interface), m_Game(m_Interface)
{
    ...
    m_Commands.emplace("save", std::make_unique<CCommandSave>(m_Interface, " [filename] : saves game to 'filename', filenames cannot contain white space", m_Game));
    m_Commands.emplace("load", std::make_unique<CCommandLoad>(m_Interface, " [filename] : loads 'filename', filenames cannot contain white space space", m_Game));
